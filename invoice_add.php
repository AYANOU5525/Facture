...
$stmt = $pdo->prepare("SELECT * FROM Produit WHERE Id_Entreprise = ? AND Quantite_En_Stock > 0 ORDER BY Nom_Produit");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $client = trim($_POST['client'] ?? 'Client Comptant');
    $items = $_POST['items'] ?? [];
    ...
    if (empty($items)) {
        ...
        try {
            ...
            foreach ($items as $item) {
                ...
                $stmt = $pdo->prepare("SELECT * FROM Produit WHERE Id_Produit = ? AND Id_Entreprise = ? FOR UPDATE");
            ...
            $stmt = $pdo->prepare("INSERT INTO Vente (Numero_Vente, Nom_Client, Articles_JSON, Montant_Total, Type_Vente, Id_Entreprise, Date_Vente) VALUES (?, ?, ?, ?, 'directe', ?, NOW())");
            $stmt->execute([$numero, $client, json_encode($articles_json, JSON_UNESCAPED_UNICODE), $total_vente, $entreprise_id]);
