...
$orderService = new OrderService($pdo, new OrderRepository($pdo));
$b2bOrderService = new B2BOrderService($pdo, new OrderRepository($pdo));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'creer_commande') {
        try {
            ...
            $commande = $b2bOrderService->create([
    ...
    elseif ($action === 'en_preparation') {
        try {
            ...
            $cmd = $orderService->transitionForSeller(
                $id_commande,
                $mon_entreprise_id,
                'validee',
                'en_preparation'
    ...
    elseif ($action === 'expedier') {
        try {
            $id_commande = intval($_POST['id_commande'] ?? 0);
            $shipment = $shipmentService->ship($id_commande, $mon_entreprise_id);
            $cmd = $shipment['order'];
    ...
    elseif ($action === 'refuser') {
        try {
            ...
            $cmd = $orderService->refuse($id_commande, $mon_entreprise_id, $motif_refus);
