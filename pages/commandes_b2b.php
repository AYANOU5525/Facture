        ...
        try {
            ...
            elseif ($_POST['action'] === 'expedier') {
                ...
                if ($cmd) {
                    ...
                    $id_vente_ins = $pdo->lastInsertId();

                    // 3. Créer la Facture (Compta)
                    $stmt = $pdo->prepare("INSERT INTO Facture (Id_Vente, Id_Commande_B2B, Numero_Facture, Date_Echeance, Montant_HT, Montant_TTC, Id_Entreprise) VALUES (?, ?, ?, DATE_ADD(NOW(), INTERVAL 30 DAY), ?, ?, ?)");
                    $stmt->execute([$id_vente_ins, $id_commande, $ref_facture, $cmd['Montant_Total'] * 0.8, $cmd['Montant_Total'], $mon_entreprise_id]);
                    $id_facture_ins = $pdo->lastInsertId();

                    // 4. Créer l'entrée Logistique
                    $stmt = $pdo->prepare("INSERT INTO Logistique (Id_Vente, Id_Commande_B2B, Id_Facture, Statut_Livraison, Id_Entreprise) VALUES (?, ?, ?, 'traitement', ?)");
                    $stmt->execute([$id_vente_ins, $id_commande, $id_facture_ins, $mon_entreprise_id]);

                    $success = "Commande expédiée. Facture N°$ref_facture et Suivi Logistique générés !";
                } else {
                    $error = "Erreur : Commande introuvable.";
                }
            }
...
<div class="container fade-in">
    ...
    <div class="row">
        <!-- NOUVELLE COMMANDE (COLONNE GAUCHE) -->
            ...
            <div class="col-md-4">
                <div class="card" style="border-left: 5px solid #3498db;">
                    ...
                    <form method="GET" action="commandes_b2b.php" style="display: flex; gap: 10px; align-items: center; margin-bottom: 20px;">
                        ...
                        <select name="vendeur" class="form-control" required>
                            ...
                            <table class="table table-sm">
                                ...
                                <tbody>
                                    <?php foreach ($produits_b2b as $p): ?>
                                        <tr>
                                            <td>
                                                <small><?= htmlspecialchars($p['Nom_Produit']) ?></small><br>
                                                <strong><?= number_format((float)($p['Prix_B2B'] ?? 0), 0, ',', ' ') ?> F</strong>
                                            </td>
                                            <td><?= $p['Quantite_En_Stock'] ?></td>
                                            <td><input type="number" name="items[<?= $p['Id_Produit'] ?>]" min="0" max="<?= $p['Quantite_En_Stock'] ?>" class="form-control form-control-sm" style="width: 60px;"></td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
        ...
        <!-- TABLEAU (COLONNE DROITE / PLEINE PAGE) -->
        <div class="<?= $onglet === 'passees' ? 'col-md-8' : 'col-md-12' ?>">
            <div class="card">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <small>
                                                ...
                                                <strong>Total: <?= number_format((float)($c['Montant_Total'] ?? 0), 0, ',', ' ') ?> F</strong>
                                            </small>
                                        </td>
                                        <td>
                                            ...
                                            <?php if ($onglet === 'recues'): ?>
                                                ...
                                                <?php elseif ($c['Statut'] === 'validee'): ?>
                                                    <form method="POST">
                                                        ...
                                                        <input type="hidden" name="id_commande" value="<?= $c['Id_Commande_B2B'] ?>">
                                                        <button class="btn btn-info btn-sm">Expédier</button>
                                                    </form>
                                                <?php endif; ?>
                                            <?php else: ?>
